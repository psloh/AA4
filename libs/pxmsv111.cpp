...
char XMSObject::AllocateEMB(int far *Handle, unsigned Size)
{
	asm {
   	mov ah, 0x09
      mov dx, Size
...
char XMSObject::ReallocateEMB(int Handle, unsigned NewSize)
{
	asm {
   	mov ah, 0x0F
      mov bx, NewSize
...
char XMSObject::FreeEMB(int far *Handle)
{
	asm {
   	mov ah, 0x0A                  /*        tweaking
      mov dx, Handle
      call [XMSDriver]                */
      les  di, [Handle]
      mov  dx, es:[di]
...
char XMSObject::CopyToEMB(void *SourcePtr, long Length, int DestHandle, long DestOffset)
{
	TempParams.Length       = ((Length+1)/2) * 2;
...
char XMSObject::CopyFromEMB(int SourceHandle, long SourceOffset, long Length, void *DestPtr)
{
	TempParams.Length       = ((Length+1)/2) * 2;
